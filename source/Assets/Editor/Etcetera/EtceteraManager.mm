...
// will ask for review if: the user didnt click 'dont ask again', it has been at least 2 days since last asking or since the first launch,
// the current version has not been reviewed and the app has been launched more than launchCount times since the last review
- (void)askForReviewWithLaunchCountLocalized:(int)launchCount hoursBetweenPrompts:(int)hoursBetweenPrompts title:(NSString*)title message:(NSString*)message iTunesUrl:(NSString*)iTunesUrl
    remindMeLater:(NSString*)remindMeLater yesRateIt:(NSString*)yesRateIt dontAskAgain:(NSString*)dontAskAgain
{
	// store this globally for easy access
	_hoursBetweenPrompts = hoursBetweenPrompts;
    ...
    // change \n (2 characters) to \n (the 1 character version)
	NSMutableString *myRepr = [[NSMutableString alloc] initWithString:message];
   ...
   // }
    [myRepr replaceOccurrencesOfString:@"\\n" withString:@"\n" options:0 range:myRange];
	UIAlertView *alert = [[UIAlertView alloc] initWithTitle:title
													message:myRepr
												   delegate:self
										  cancelButtonTitle:remindMeLater
										  otherButtonTitles:yesRateIt,
                                                dontAskAgain, nil];
	alert.tag = kRTAAlertTag;
	[alert show];
	[alert release];

...
public class AmpBundleManager : MonoBehaviour
{
	...
	public bool bundleVersioncheck = false;
	private bool bundleVersioncheckInProgress = false;
	//private GameObject bundleVersioncheckMsgObbect = null;	
	private GameObject ampAssetListReuqestMsgObject = null;
	...
	{
		if (Settings.GetBool("embed-all-envsets", false) && Settings.GetBool("skip-bundle-check-if-embed-all", true))	//false))
		{
	...
	/// </param>
	IEnumerator StartBundleVersionCheck()
	{
		if( bundleVersioncheck )
		{
			...
			string url = "http://oz.i.dol.cn/android/oz/0158-0006/assert";
			...
			if ( noErrors )
			{
				...
				catch (System.Exception e)
				{
					...
					ShowAMPSDownloadError( "Msg_DownloadFailLong", "UIManagerOz::StartBundleVersionCheck: deserialization exception. url=" + url + " exception=" + e );
					noErrors = false;
				}
			}
	...
	bool UpdateBundleVersion(string bundleName)
	{
		if( bundleName != "" )
		{
			...
			//{
				DownloadedAssetBundleInfo info = ResourceManager.SharedInstance.downloadedAssetBundles.FindInfo(bundleName);
				if( info != null )
				{
					int v = ampAssetBundleData.GetAssetVersion(bundleName);
					notify.Debug("ver = " + v);
	...
	bool GetVersionNumbers( WWW www, bool noErrors, object results)
	{
...
//				object query = rootDict["querySuccess"];
				...
				Dictionary<string, object> resultData = rootDict["result"] as Dictionary<string, object>;
				
				if(resultData!=null)
				{
					List<object> bItems = resultData["data"] as List<object>;
					notify.Debug("Parsing list len = " + bItems.Count);
					if (bItems.Count > 0)
					{
						AMPBundleData [] bundleList = new AMPBundleData[bItems.Count];
						ampAssetBundleData.bundleList = bundleList;
						
						for( int i = 0 ; i < bItems.Count; i++)
						{
							Dictionary<string, object> bd = bItems[i] as Dictionary<string, object>;
							result = true;
						
							AMPBundleData adata = new AMPBundleData();
						...
						UpdateBundleVersion( Localization.SharedInstance.GetAssetBundleName(Localization.SharedInstance.GetLangBySystem()));
						// check hires 
						UpdateBundleVersion( DownloadManagerUI.bundleName);
						// env
						foreach (KeyValuePair<int,EnvironmentSetData> kvp in EnvironmentSetManager.SharedInstance.AllDict)
						{
							if (!kvp.Value.IsEmbeddedAsResource())		// if not embedded resource / asset bundle, update version info
							{
	...
	{
		ampAssetListReuqestMsgObject = obj;// this is a blocker path, only be called when the bundle is not loaded before
	...
	public void ShowAMPSDownloadError( string localizedMessageKey, string debugMessage )
	{
		...
		System.Exception exception = new System.Exception( debugMessage );
...
public class AMPBundleData
{
	...
	public int GetAssetVersion()
	{
		...
		{
			UIManagerOz.notify.Warning("Asset version number parse error [" + assetVersion + "] exception " + theException.Message);
		}
...
{
	public AMPBundleData [] bundleList = null;
	...
	public int GetAssetVersion(string bname)
	{
		...
		int latestVersion = -1;
		string fullname = AssetBundleLoader.GetFullAssetBundleName(bname);
		
		foreach (AMPBundleData ad in bundleList)
		{
			if (ad != null && ad.fileName == fullname)
			{
				int version = ad.GetAssetVersion();
	...
	public string GetCdnUrl(string basename)
	{
		...
		string fullname = AssetBundleLoader.GetFullAssetBundleName(basename);

...
public  class DebugConsoleOz : DebugConsole {
	...
	public  static new DebugConsoleOz Instance
	{
		get
		{
			if (_instance == null)
	...
	static DebugConsoleOz() 
	{
		...
		notify.Debug("_instance = " + _instance.ToString());
	...
	private static bool reset(string cmd)
	{
		...
		GameProfile gp = UnityEngine.Object.FindObjectOfType(typeof(GameProfile)) as GameProfile;
		if(gp) {
	...
	private static bool ft(string cmd)
	{
		...
		else if (GameController.SharedInstance.IsGameStarted && ! ( GameController.SharedInstance.IsGameStarted && GameController.SharedInstance.IsGameOver)) 
		{
			typedString = "You can not fast travel while you are playing";			
		}
		else
		{
			string destCode = splits[1];
	...
	private static bool lang(string cmd)
	{
		...
		else
		{
			...
			switch(lang){
				case "en" : Localization.SharedInstance.SetLanguage("English");
				break;
				case "fr" : Localization.SharedInstance.SetLanguage("French");
				break;
				case "nl" : Localization.SharedInstance.SetLanguage("Dutch");
				break;
				case "de" : Localization.SharedInstance.SetLanguage("German");
				break;
				case "it" : Localization.SharedInstance.SetLanguage("Italian");
				break;
				case "es" : Localization.SharedInstance.SetLanguage("Spanish");
				break;
				case "ru" : Localization.SharedInstance.SetLanguage("Russian");
				break;
				case "ja" : Localization.SharedInstance.SetLanguage("Japanese");
				break;
				case "zh" : Localization.SharedInstance.SetLanguage("Chinese");
				break;
				case "zht" : Localization.SharedInstance.SetLanguage("Chinese_Traditional");
				break;
				case "pt" : Localization.SharedInstance.SetLanguage("Portuguese");
				break;
				case "ko" : Localization.SharedInstance.SetLanguage("Korean");
				break;
			}
		}
		return result;
	...
	{
		DebugConsoleOzGui consoleGUI = Services.Get<DebugConsoleOzGui>();
		if(consoleGUI != null)
		{
			consoleGUI.enabled = false;
		}
		HUDFPSUnityGui fpsGUI = Services.Get<HUDFPSUnityGui>();
		if(fpsGUI != null)
		{
...
#pragma warning restore 0162
	...
	private static bool unlockallobjectives(string cmd)
	{
		...
		for(int i=0;i<ObjectivesManager.Objectives.Count;i++)
			GameProfile.SharedInstance.Player.objectivesEarned.Add(ObjectivesManager.Objectives[i]._id);
	...
	private static bool completeobjective(string cmd)
	{
		...
		if(ind>=0 && ind<GameProfile.SharedInstance.Player.objectivesActive.Count) {
			ObjectiveProtoData data = GameProfile.SharedInstance.Player.objectivesActive[ind];
			if(data!=null)
			{
				data._conditionList[0]._earnedStatValue = data._conditionList[0]._statValue;
	...
	{
		foreach(ObjectiveProtoData opd in ObjectivesManager.Objectives)
		{
		...
		typedString = "Objectives of difficulty 1 through 3 have been marked complete! Complete current objectives to get new ones.";
		return true;
	}
	...
	private static bool completeobjdiff4to5(string cmd)
	{
		...
		typedString = "Objectives of difficulty 4 through 5 have been marked complete! Complete current objectives to get new ones.";
		return true;
	}
	...
	private static bool completeobjdiff6to7(string cmd)
	{
		...
		typedString = "Objectives of difficulty 6 through 7 have been marked complete! Complete current objectives to get new ones.";
		return true;
	}

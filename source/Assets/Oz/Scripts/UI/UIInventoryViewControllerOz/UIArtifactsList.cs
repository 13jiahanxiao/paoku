...
public class UIArtifactsList : MonoBehaviour 
{
	...
	private ArtifactCellData artifactToPurchase;	//OrGem;	// temp reference, for use when purchasing or gemming a specific item
	...
	{ 
		...
		grid = gameObject.transform.Find("artifactGrid").gameObject;			// connect to this panel's grid automatically			
	...
	private List<GameObject> CreateCells()
	{
		...
		{
			GameObject panel = CreatePanel(artifactData, grid);
	...
	{
		GameObject obj = (GameObject)Instantiate(Resources.Load("ArtifactStoreCellOz"));		// instantiate objective cell from prefab
		...
		obj.GetComponent<ArtifactCellData>().viewController = viewController;	// pass on reference to view controller, for event response
		obj.GetComponent<ArtifactCellData>().scrollList = this.gameObject;
	...
	{
		ArtifactCellData artifactCellData = button.transform.parent.parent.parent.GetComponent<ArtifactCellData>();
...
//		GoogleIABManager.artifactPurchaseFailedEvent += onPurchaseFailedByRMB;
	...
	{
		UnityEngine.Debug.Log("~~~~~~~~buyArtifactByCoins");
		...
		if (playerStats.IsArtifactMaxedOut(artifactID) == false) 	// check if already purchased maximum allowed
		{
			...
			if (playerStats.CanAffordArtifact(artifactID) == true)
			{
				//UIConfirmDialogOz.onNegativeResponse += OnPurchaseNo;
				//UIConfirmDialogOz.onPositiveResponse += OnPurchaseYes;

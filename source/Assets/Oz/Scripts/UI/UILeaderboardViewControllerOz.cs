...
public enum SocialScreenName { TopDistances, TopScores, }	//SocialConnections, TopScores, TopDistances, Challenges }
...
public class UILeaderboardViewControllerOz : UIViewControllerOz
{
	...
	public int GetkLeaderboardLabelCount() { return kLeaderboardLabelCount; }
	...
	private Dictionary<string, Texture2D> profilePhotos = new Dictionary<string, Texture2D>();
	...
	public void OnLeaderboardLoad()
	{
		...
		if (Initializer.SharedInstance.GetDisplayLeaderboard()
			&& UIManagerOz.SharedInstance.leaderboardVC.gameObject.active)
		{
	...
	{
		foreach (UILeaderboardList list in pageLists)
			list.gameObject.GetComponent<UIDraggablePanel>().ResetPosition();
	}	
	...
	private void ShowLeaderboards(bool valid, SocialScreenName page)
	{
		...
		NGUITools.SetActive(graphicsGOs[(int)SocialScreenName.TopDistances], (valid && page == SocialScreenName.TopDistances));
	...
	private void SetArrows(bool valid, SocialScreenName page)
	{
		...
		NotificationSystem notSys = Services.Get<NotificationSystem>();
		int scoreWentUpCount = notSys.GetOneShotCount(OneShotNotificationType.TopScorePositionWentUp);
		int scoreWentDownCount = notSys.GetOneShotCount(OneShotNotificationType.TopScorePositionWentDown);
		int distanceWentUpCount = notSys.GetOneShotCount(OneShotNotificationType.TopDistancePositionWentUp);
		int distanceWentDownCount = notSys.GetOneShotCount(OneShotNotificationType.TopDistancePositionWentDown)	;
		...
		if (valid)
		{
			...
			if (page == SocialScreenName.TopScores)
			{
				...
				{
					SetArrow(page, rank, true, OneShotNotificationType.TopScorePositionWentUp, OneShotNotificationType.TopScorePositionWentDown);
				}
				else if (scoreWentDownCount > 0)
				{
					SetArrow(page, rank, false, OneShotNotificationType.TopScorePositionWentUp, OneShotNotificationType.TopScorePositionWentDown);
				}				
			}
			else if (page == SocialScreenName.TopDistances)
			{
				...
				{
					SetArrow(page, rank, true, OneShotNotificationType.TopDistancePositionWentUp, OneShotNotificationType.TopDistancePositionWentDown);
				}
				else if (distanceWentDownCount > 0)
				{
					SetArrow(page, rank, false, OneShotNotificationType.TopDistancePositionWentUp, OneShotNotificationType.TopDistancePositionWentDown);
				}	
			}
		}
	}
	
	private void SetArrow(SocialScreenName page, int rank, bool status, OneShotNotificationType clear1, OneShotNotificationType clear2)
	{
		notify.Debug("showing: " + page.ToString() + " with status: " + status.ToString());
		PositionArrow(status, rank-1, page);
		Services.Get<NotificationSystem>().SetNotificationIconsForThisPage(UiScreenName.LEADERBOARDS);					
		notify.Debug("clearing: " + clear1.ToString());
		notify.Debug("clearing: " + clear2.ToString());
	...
	private void SwitchToPanel(SocialScreenName panelScreenName)	//, bool isInitialClick = false )	// activate panel upon button selection, passing in SocialScreenName
	...
	// set only appropriate panel active, make others inactive
		...
		pageLists[(int)pageToLoad].GetComponent<UIDraggablePanel>().ResetPosition();
	}	
	...
	private void RefreshLeaderboardData()
	{
		...
		// --- Top Scores -----------------------------------------------------
		List<LeaderboardManager.LeaderboardEntry> topScores = leaderboardManager.GetTopScores( kLeaderboardLabelCount );
		...
		// --- Top Distances --------------------------------------------------
		List<LeaderboardManager.LeaderboardEntry> topDistances = leaderboardManager.GetTopDistances( kLeaderboardLabelCount );
...
//		foreach (UILeaderboardList list in pageLists)
//		{
//			list.playerCellRoot.gameObject.SetActiveRecursively(false);
//		}
//	}

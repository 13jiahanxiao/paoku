...
public class UIIAPMiniViewControllerOz : UIViewControllerOz
{
	...
	public override void appear() 
	{
		...
		UIManagerOz.SharedInstance.UICamera.GetComponent<UICamera>().clipRaycasts = false;
...
//			"oncompletetarget", gameObject));	
	...
	{
		UIManagerOz.SharedInstance.UICamera.GetComponent<UICamera>().clipRaycasts = true;
		base.disappear();
	}
	
	private void CreateStore()
	{
		storePanelGOs[(int)pageToLoad].GetComponent<UIDraggablePanel>().ResetPosition();
	...
	//		return;
		...
		if (pageToLoadIfMoreSpecificNeeded == ShopScreenName.Gems)
		{
			...
			else if (comingFromResurrectMenu == false)	// for start of run
			{
				...
				UIManagerOz.SharedInstance.inGameVC.OnUnPaused();	

		...
		{	
			...
			//	tween.callWhenFinished = "SwapInNewObjective";	//CheckIfLevelCompleted
				...
				if (AudioManager.Instance!=null)	
					AudioManager.Instance.GetSoundEffectsPlayer().PlayOneShot(AudioManager.Instance.ScoreBlast, 1.0f);	// play sound effect				
...
//					//AnimateRankProgressBar(GameProfile.SharedInstance.Player.GetCurrentRankProgress(), 1.0f);		// animate to reflect progress
	...
	private void GetWeeklyChallengeReward(ObjectiveProtoData challengeData)
	{
		...
		string iconName = ObjectivesManager.GetRewardIconName((int)challengeData._rewardType);
		...
		if (AudioManager.SharedInstance!=null)
			AudioManager.Instance.GetSoundEffectsPlayer().PlayOneShot(AudioManager.Instance.AngelWings, 1.0f);	// play sound effect
		...
		// consumables
		else if (currentRewardType == RankRewardType.HeadStartConsumables) 
			GameProfile.SharedInstance.Player.EarnConsumable(UIManagerOz.SharedInstance.inGameVC.bonusButtons.HeadStartID, currentRewardItemID);
		else if (currentRewardType == RankRewardType.MegaHeadStartConsumables) 
			GameProfile.SharedInstance.Player.EarnConsumable(UIManagerOz.SharedInstance.inGameVC.bonusButtons.BigHeadStartID, currentRewardItemID);					
		else if (currentRewardType == RankRewardType.NoStumbleConsumables) 
			GameProfile.SharedInstance.Player.EarnConsumable(UIManagerOz.SharedInstance.inGameVC.bonusButtons.StumbleProofID, currentRewardItemID);
		else if (currentRewardType == RankRewardType.ThirdEyeConsumables) 		
			GameProfile.SharedInstance.Player.EarnConsumable(UIManagerOz.SharedInstance.inGameVC.bonusButtons.ThirdEyeID, currentRewardItemID);
		else if (currentRewardType == RankRewardType.ExtraMultiplierConsumables) 	
			GameProfile.SharedInstance.Player.EarnConsumable(UIManagerOz.SharedInstance.inGameVC.bonusButtons.BonusMultiplierID, currentRewardItemID);
	...
	{
		if ( gameObject.active && TimerLabel != null )
		{

...
{	
	...
	public void EnterStatsPage()	
	{
		...
		int temp = GameProfile.SharedInstance.GetTemporaryScoreMultiplier();
		MultiplierLabel.text = (GameProfile.SharedInstance.GetPermanentScoreMultiplier() + temp).ToString() + "x";
		//if(temp>0)
	...
	public void Reset(){
		if(firstLoad){
			firstLoad = false;
			longestRunScale = longestRun.transform.localScale;
			distSliderScale = distSlider.transform.localScale;
			newHighScoreScale = newHighScore.transform.localScale;
			scoreSliderScale = scoreSlider.transform.localScale;
			iconGemScale = iconGem.transform.localScale;
			iconDistScale = iconDist.transform.localScale;
			iconMultScale = iconMult.transform.localScale;
			iconCoinsScale = iconCoins.transform.localScale;
		}
		EnterStatsPage();
		bestRun = false;
		bestScore = false;
		...
		longestRun.transform.localScale = longestRunScale;
		distSlider.sliderValue = 0f;
		
		newHighScore.transform.localScale = newHighScoreScale;
		scoreSlider.transform.localScale = scoreSliderScale;
		scoreSlider.sliderValue = 0f;
		...
		ScoreLabel.transform.localScale = new Vector3(65f,65f,1f);
		ScoreLabel.text = "0";
		//DistanceLabel.MakePixelPerfect();
		DistanceLabel.transform.localScale  = new Vector3(50f,50f,1f);
	...
	public void AnimateDistUpdate(float val){
		...
		distSlider.sliderValue = bestRunRatio * val;
...
//			}
			...
			iTween.ScaleTo(DistanceLabel.gameObject, iTween.Hash(
				"time", 0.3f,
				"scale",  DistanceLabel.transform.localScale * 1.4f,
	...
	public void ShowLongestRun(){		
		...
		iTween.ScaleFrom(longestRun.gameObject, iTween.Hash(
			"time", 0.3f,
			"scale", longestRun.transform.localScale * 2f,
	...
	public void AnimateScoreUpdate(float val){
		...
		scoreSlider.sliderValue = bestScoreRatio * val;
		if(val >= 1){
			if(bestScore){		
				ParticleSystem[] fx = ScoreLabel.transform.GetComponentsInChildren<ParticleSystem>();
				foreach (ParticleSystem item in fx)
			...
			iTween.ScaleTo(ScoreLabel.gameObject, iTween.Hash(
				"time", 0.3f,
				"scale",  ScoreLabel.transform.localScale * 1.3f,
				"easetype", iTween.EaseType.easeOutBack,
	...
	public void ShowNewHighScore(){
		...
		iTween.ScaleFrom(newHighScore.gameObject, iTween.Hash(
			"time", 0.3f,
			"scale", newHighScore.transform.localScale * 1.4f,

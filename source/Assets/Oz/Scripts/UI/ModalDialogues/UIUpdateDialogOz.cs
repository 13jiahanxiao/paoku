...
//			progressBar.sliderValue = p;
		...
		if (progressBar && progressBar.sliderValue < p)
		{
			progressBar.sliderValue = p;						// update progress bar, but only if it's moving to the right
		}	
	}
	
	void Update()
	{
		if (lastSliderValue != progressBar.sliderValue)			// check if it's stuck
		{
		...
		lastSliderValue = progressBar.sliderValue;				// store the previous value
	}
	...
	public void StartPrompt(GameObject messageobj, bool forcedownload, bool noDownloadOKPrompt)
	{
		...
		progressBar.sliderValue = 0f;		// reset progress bar value
...
//		closeButton.SetActiveRecursively(false);
	//	okButton.SetActiveRecursively(false);

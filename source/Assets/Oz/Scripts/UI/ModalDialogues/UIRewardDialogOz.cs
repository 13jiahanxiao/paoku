...
public class UIRewardDialogOz : UIModalDialogOz	//MonoBehaviour
{
	...
	public ParticleSystem iconGlow;
	public ParticleSystem iconStars;	
	
	//public void ShowRewardDialog(string title, string description, int quantity, string itemIconName, GameObject msgObject = null) 
	public void ShowRewardDialog(string title, string itemIconName, string positiveButtonText, GameObject msgObject = null) 
	{
		...
		if (iconGlow && iconGlow.gameObject.active) iconGlow.Play();
		if (iconStars && iconStars.gameObject.active) iconStars.Play();
	...
	public void OnCenterButtonPress()
	{
		...
		if (iconGlow && iconGlow.gameObject.active) 
		{
			iconGlow.Stop();
			iconGlow.Clear();
		}
		
		if (iconStars && iconStars.gameObject.active)
		{
			iconStars.Stop();
			iconStars.Clear();
		}

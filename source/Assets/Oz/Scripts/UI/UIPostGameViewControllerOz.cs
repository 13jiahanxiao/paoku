...
public class UIPostGameViewControllerOz : UIViewControllerOz
{
	...
	private StatsRoot statsRootCmpt;
	public GameObject statsButton;
	//public GameObject bottomPanel; //- commented out for now because of nulls N.N.
	...
	public List<UISlicedSprite>  slicedSprites = new List<UISlicedSprite>();
	...
	{
		//notify.Debug("UIPostGameViewControllerOz.start");
		base.Start();
	...
	//	UIWidget[] trs = bottomPanelGOs[0].transform.GetComponentsInChildren<UIWidget>();
	//	foreach(UIWidget ui in trs){
	//		Debug.Log(ui.name + " " + ui.material.renderQueue);
	//	}
	}
	...
	{		
		...
		rewardPanel.SetActiveRecursively(false);
...
//	public void ShowObjectivesPage()
//	{
//		notify.Debug("ShowObjectivesPage");
//		canSwitchPages = false;
...
//		MoveTweener(weeklyRoot, new Vector3(-1000.0f, weeklyRoot.transform.localPosition.y, 0.0f), tweenTime);
//	}	
...
//			"islocal", true));
	...
	public void ShowBottomPanelUpdate(float val)
	{
		...
		/*
		if(UIManagerOz.SharedInstance.chosenUIResType == UIManagerOz.UIResolutionType.kResolution480){ // only needed if low-end devices
			foreach (UISlicedSprite ss in slicedSprites){
				ss.fillCenter = !ss.fillCenter; // force update
			}
			if(val >= 1.0f){
				foreach (UISlicedSprite ss in slicedSprites){
					ss.fillCenter = false;
	...
	public void OnEscapeButtonClicked()
	{
		...
		{
			OnMenuClicked();	// mapped to Android hardware 'back' button, in this case take us back to main menu
		}
		else
		{
			OnSpeedUpButton();	// speed up animations
		}
	}		
#endif	
	...
	{
		if( rewardPanel.gameObject.active == false ) return ;
	...
	public void ShowRewardPanel()
	{
		...
		rewardPanel.SetActiveRecursively(true);
		rewardIcon.alpha = 0f;
...
//				"islocal", true));
	...
	public void OnMenuClicked()		//OnHomeButtonClicked()
	{
		//		AnalyticsInterface.LogNavigationActionEvent( "Menu", "Post Run", "Main Menu" );
		...
		//paperViewController.ShowPaperVC();	//SetToPostRun();
		disappear();	
	}
	...
	public void OnUpgradesClicked()
	{
		...
		UIManagerOz.SharedInstance.PaperVC.appear();	
		//UIManagerOz.SharedInstance.PaperVC.GoToUpgrades(UpgradesScreenName.PowerUps);
		//UIManagerOz.SharedInstance.mainVC.appear();
		disappear();
	}	
	
	public void OnShareClicked() //- commented out for now because of nulls N.N.
	{
	...
	private void OnPlayClicked() 
	{
		...
		//		AnalyticsInterface.LogNavigationActionEvent( "Play", "Post Run", "Game" );

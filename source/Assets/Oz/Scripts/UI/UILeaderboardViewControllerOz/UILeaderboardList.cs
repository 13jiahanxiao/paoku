...
public class UILeaderboardList : MonoBehaviour 
{
	...
	public void Refresh(List<LeaderboardManager.LeaderboardEntry> topValues)
	{
		...
		foreach (LeaderboardCellData childCell in childCells)
		{
	...
	public void TurnOffAllArrows()
	{
		...
		foreach (LeaderboardCellData child in childCells)	//foreach (GameObject child in childCells)
		{
	...
	public void CancelProfilePhotoDownloads()
	{
		...
		foreach (LeaderboardCellData child in childCells)
		{
	...
	private List<LeaderboardCellData> CreateCells()
	{
		...
		List<LeaderboardCellData> newObjs = new List<LeaderboardCellData>();

		foreach (LeaderboardManager.LeaderboardEntry data in dataList)
		{
			LeaderboardCellData panel = CreatePanel(data, grid);	//GameObject panel = CreatePanel(data, grid);
	...
	private void DestroyCells()
	{
		...
		foreach (LeaderboardCellData childCell in childCells)		//foreach (GameObject childCell in childCells)
		{
	...
	private string GenerateCellLabel(LeaderboardManager.LeaderboardEntry data)
	{
		...
		bool isScores = (listType == SocialScreenName.TopScores);	// true = scores, false = distances;
		int rank = isScores ? data.rankScore : data.rankDistance;
		
		return ("Cell_" + rank.ToString("D8"));
	}

	private LeaderboardCellData CreatePanel(LeaderboardManager.LeaderboardEntry _data, GameObject _grid)	//, string cellName)
	{
		notify.Debug( "[UILeaderboardList] CreatePanel" );
		
		GameObject obj = (GameObject)Instantiate(Resources.Load("LeaderboardsPlayerCellOz"));	// instantiate objective cell from prefab
		obj.transform.parent = _grid.transform;									// null passed in for parentCell, which means this is a regular size cell
		obj.transform.localScale = Vector3.one;
		obj.transform.rotation = grid.transform.rotation;
		obj.transform.localPosition = Vector3.zero;
		
		LeaderboardCellData cellData = obj.GetComponent<LeaderboardCellData>();

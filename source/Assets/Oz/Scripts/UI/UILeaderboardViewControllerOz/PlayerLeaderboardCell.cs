...
public class PlayerLeaderboardCell : MonoBehaviour 
{
		...
		PlayerLeaderboardCell playerCell, LeaderboardManager.LeaderboardEntry playerData)
	{
		...
		if (rank > 5)
		{
			...
			clippedPanel.clipRange = new Vector4(clippedPanel.clipRange.x, clippedPanel.clipRange.y, clippedPanel.clipRange.z, fourCellClipYScale);
			
			playerCellRoot.SetCellVisible(true);	//playerCell.gameObject.SetActiveRecursively(true);
		...
		else
		{
			...
			clippedPanel.clipRange = new Vector4(clippedPanel.clipRange.x, clippedPanel.clipRange.y, clippedPanel.clipRange.z, fiveCellClipYScale);
			playerCellRoot.SetCellVisible(false);	// playerCell.gameObject.SetActiveRecursively(false);
		}
	}
}

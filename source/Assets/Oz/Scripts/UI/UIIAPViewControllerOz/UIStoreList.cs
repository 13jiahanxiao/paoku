...
public enum StoreItemType { CoinBundle, GemBundle, Wallpaper, Character, MovieTickets, CoinOffers, }	// Burstly now handles Wallpaper, MovieTickets and CoinOffers
...
public class UIStoreList : MonoBehaviour 
{
	...
	private List<IAP_DATA> myDataList = new List<IAP_DATA>();	//private List<StoreItem> myDataList = new List<StoreItem>();
	...
	public static bool storeLoaded = false;						// for product list loaded from store via network
	public static bool fullStoreScrollListGenerated = false;	// for scroll list in regular 'more coins' store
	public static bool miniStoreScrollListGenerated = false;	// for scroll list in IAPminiStore
...
#if UNITY_ANDROID
		Invoke("HideBusyIndicator", 0.1f);	// unhiding the busy indicator is broken on android on some devices, need slight delay
#endif		
		connectingToStore.enabled = false;	// hide 'connecting to store' text
...
//		else
	...
	public void ReceivedProductList()	// store list received	
	{
		...
		if (gameObject.active)	// only generate scroll list if this page is still being displayed
		{
			GenerateScrollList();
		}
	...
	{
		notify.Debug("GetFullStoreItemList called in UIStoreList, with IAPWrapper.iapTable.Count = " + IAPWrapper.iapTable.Count);
		
		List<IAP_DATA> itemsList = new List<IAP_DATA>();
		
		UnityEngine.Debug.Log("iapTable:"+IAPWrapper.iapTable.Count);
		foreach (KeyValuePair<string,IAP_DATA> storeItem in IAPWrapper.iapTable)	//sortedDataList)
		{
			
				List<WeeklyDiscountProtoData> weeklyDiscountList = Services.Get<Store>().GetWeeklyDiscounts();
				
				IAP_DATA storeIap = storeItem.Value;
				
				foreach (WeeklyDiscountProtoData discount in weeklyDiscountList)
				{
					foreach (DiscountItemProtoData discountItem in discount._itemList)
					{
...
//		grid.GetComponent<UITable>().Reposition();		
	...
	private List<GameObject> CreateCells()
	{
		...
		foreach (IAP_DATA data in sortedDataList)	// myDataList)
	...
	private GameObject CreateStoreItemPanel(IAP_DATA storeItemData)	
	{
		...
		GameObject obj = (GameObject)Instantiate(Resources.Load("StoreCellOzNew"));	
		//end
...
//		obj.transform.rotation = grid.transform.rotation;
		...
		obj.GetComponent<StoreCellData>().viewController = viewController;	// pass on reference to view controller, for event response	
		obj.GetComponent<StoreCellData>().scrollList = this.gameObject;
...
//	}
	...
	public void OnStoreBuyPressed(GameObject cell) 
	{
		...
		Services.Get<NotificationSystem>().ClearNotification(NotificationType.MoreCoins, 0);		//storeItemToPurchase._data.productID);
...
//		UIManagerOz.SharedInstance.confirmDialog.ShowConfirmDialog(prompt, "Btn_No", "Btn_Yes");
	...
	public void CancelContinue() 	// if in-game, in resurrect menu, on negative response, kill the mini store and go to gatcha 	//end game
	{
		UIConfirmDialogOz.onNegativeResponse -= CancelContinue;
		UIConfirmDialogOz.onPositiveResponse -= DoContinue; 			
		#pragma warning disable

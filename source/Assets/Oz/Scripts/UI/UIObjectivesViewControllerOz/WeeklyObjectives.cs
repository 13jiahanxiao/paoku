...
public class WeeklyObjectives : MonoBehaviour 	// load 'weekly' objectives from the web (called 'challenges')
{
	...
	*/
	...
	private static int _weeklyObjectiveCount = 0;
	public static int WeeklyObjectiveCount
	{
		get
		{
			return _weeklyObjectiveCount;
		}
	}
	
	private static int _teamObjectiveCount = 0;
	public static int TeamObjectiveCount
	{
		get
		{
			return _teamObjectiveCount;
		}
	}
	
	private static bool _areChallengesExpired = false;
	
	public static bool AreChallengesExpired
	{
		get
		{
			return _areChallengesExpired;
		}
	}
	...
	private List<ObjectiveProtoData> weeklyObjectivesList = new List<ObjectiveProtoData>();
	...
	private List<ObjectiveProtoData> _teamObjectiveList = new List<ObjectiveProtoData>();
	public List<ObjectiveProtoData> TeamObjectiveList
	{
		get
		{
			return _teamObjectiveList;
		}
	}
	
	private List<int> _earnedTeamObjectiveList = new List<int>();
	public List<int> EarnedTeamObjectiveList
	{
		get
		{
			return _earnedTeamObjectiveList;
		}
	}
	
	private List<int> earnedWeeklyObjectiveList = new List<int>();
	...
	public void SaveChallenges()
	{
		...
		//	Debug.Log("SAVE!");
			string fileName = Application.persistentDataPath + Path.DirectorySeparatorChar + 
				"weeklyobject.txt";
			List<object> challengeList = new List<object>();
			foreach (ObjectiveProtoData data in weeklyObjectivesList)
			{
				challengeList.Add(data.ToWebObjectiveDict());	
			}
			
			List<object> teamChallengeList = new List<object>();
			foreach ( ObjectiveProtoData data in _teamObjectiveList )
			{
...
//				Dictionary<string, object> loadedData = 
			...
			if (loadedData != null)
			{
				...
				List<object> loadObjList
					= dataDict["challengesActiveData"] as List<object>;
	
				foreach(object loadObj in loadObjList)
				{
					ObjectiveProtoData weekObj = DecodeChallengeJsonObject(loadObj);
				...
				List<object> earnedList = dataDict["challengesEarned"] as List<object>;
				
				int count = 0;
				foreach (object earnedObj in earnedList)
				{
					int earnedIndex;
				...
				{
					List<object> teamChallengeObjectList = dataDict["teamObjective"] as List<object>;
					
					if ( teamChallengeObjectList != null )
					{
						foreach ( object teamChallengeObject in teamChallengeObjectList )
						{
							ObjectiveProtoData teamChallenge = DecodeChallengeJsonObject( teamChallengeObject );
				...
				{
					List<object> earnedTeamChallengeList = dataDict["earnedTeamObjective"] as List<object>;
					
					if ( earnedTeamChallengeList != null )
					{
						
						foreach ( object earnedTeamIndexObject in earnedTeamChallengeList )
						{
							int earnedTeamIndex = 0;
	...
	//	List<int> objIdList = new List<int>();
		...
		List<ObjectiveProtoData> toRemove = new List<ObjectiveProtoData>();
		...
		foreach(ObjectiveProtoData data in toRemove)
		{
		...
		foreach ( ObjectiveProtoData teamObjective in _teamObjectiveList )
		{
...
*/		
		...
		if (noErrors)
		{
			...
			{ 
				...
				List<object> webChallengeList = dataDict["challenges"] as List<object>;
	...
	private void _reconcileChallenges( 
		...
		GameProfile gameProfile = GameProfile.SharedInstance;
		...
		foreach ( ObjectiveProtoData remoteObjective in remoteObjectiveList )
		{
			if ( localObjectiveList != null && localObjectiveList.Count > 0 )
			{
				...
				foreach ( ObjectiveProtoData localObjective in localObjectiveList )
				{
					...
					if ( localObjective._id == remoteObjective._id 
						...
						foreach ( ConditionProtoData remoteCondition in remoteObjective._conditionList )
						{
							foreach( ConditionProtoData localCondition in localObjective._conditionList )
							{
		...
		List<int> localObjectiveIndexList = new List<int>();
		...
		if ( isTeamChallenge )	
		{
			List<int> tempEarnIndexList = new List<int>();
			
			foreach ( int earnedIndex in earnedLocalObjectiveList )
			{
			...
			foreach ( int tempEarnIndex in tempEarnIndexList )
			{
		...
		{	
			...
			if ( !localObjectiveIndexList.Contains( remoteObjective._id ) )
			{
				...
				if ( isTeamChallenge )
				{
					Services.Get<NotificationSystem>().SendOneShotNotificationEvent( remoteObjective._id, (int) OneShotNotificationType.NewTeamChallenge );
				}
				else
				{
					Services.Get<NotificationSystem>().SendOneShotNotificationEvent( remoteObjective._id, (int) OneShotNotificationType.NewWeeklyChallenge );
				}
			}
		}
	...
	public bool ApplyChallengesFromInit(List<object> webChallengeList, int responseCode)
	{
		...
		{ 
			...
			List<ObjectiveProtoData> webChallengeObjList = new List<ObjectiveProtoData>();
			
			List<ObjectiveProtoData> teamWebChallengeObjList = new List<ObjectiveProtoData>();
		...
		_teamObjectiveCount = _teamObjectiveList.Count;
	...
	public void CheckExpired()
	{
		if (weeklyObjectivesList != null && weeklyObjectivesList.Count != 0)
		{
			foreach (ObjectiveProtoData weeklyObjective in weeklyObjectivesList)
			{
	...
	public void CompleteChallenges ()
	{
		...
		foreach (ObjectiveProtoData weeklyChallenge in weeklyObjectivesList)
		{
			if (!earnedWeeklyObjectiveList.Contains(weeklyChallenge._id)){
				int totalEarnedSv = 0;
				int totalSv = 0;
				int totalNeighborSv = 0;
				
				notify.Debug("weeklyChallenge: " + weeklyChallenge.ToJson());
			
				foreach(ConditionProtoData cond in weeklyChallenge._conditionList)
				{
	...
	{		
		Dictionary<string, object> dict = _challengeItem as Dictionary<string, object>;
		ObjectiveProtoData challenge = new ObjectiveProtoData(dict);
		return challenge;
	}
		...
		ObjectiveFilterType filterType, int incrementValue)
	{
		if(timeType == ObjectiveTimeType.LifeTime) {
			...
			foreach (ObjectiveProtoData challengeOb in weeklyObjectivesList) {
				if(challengeOb == null)
					continue;
				...
				int condMax = challengeOb._conditionList.Count;
				
				for (int j = 0; j < condMax; j++)
				{
					ConditionProtoData conditionOb = challengeOb._conditionList[j];

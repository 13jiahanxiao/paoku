...
{
	public UIFilledSprite progressBarFill;				// green
	public UIFilledSprite progressBarNewEarnedFill;		// yellow
	...
	// wxj
	public UISysFontLabel titleLabelSysFont, descLabelSysFont, actiTitleLabel;
	// wxj
	...
	void Awake()
	{
		...
		progressBarFill = transform.Find("SlicedSpriteFill01").GetComponent<UIFilledSprite>();
		Transform fillNew = transform.Find("SlicedSpriteFill02");
		
		if (fillNew)
			progressBarNewEarnedFill = fillNew.GetComponent<UIFilledSprite>();
...
//		notify.Debug( string.Format("[ObjectiveCellData] : UpdateIcons for: {0} Reward Icon Now: {1}", objectiveList, rewardIcon.spriteName ) );
		...
		if ( objectiveList == "weeklyPostRun" )
		{
			...
			gameObject.transform.Find("RewardIcon").GetComponent<UISprite>().spriteName = ObjectivesManager.GetRewardIconName( (int) _data._rewardType );
		}
	...
	private void OnCollectButtonPressed()
	{
		...
		if (GameProfile.SharedInstance.Player.objectivesUnclaimed.Contains(_data._id))
		{
			...
			ObjectivesManager.RewardActivityObjective(_data._id);
	...
	private void ShowRewardDialog()
	{
		...
		string rewardIconSpriteName = ObjectivesManager.GetRewardIconName( (int) _data._rewardType );
	...
	*/
		...
		//Debug.LogWarning("OnCellPressed() called for cell with id: " + _data._id.ToString());

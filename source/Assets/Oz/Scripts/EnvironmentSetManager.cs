...
public class EnvironmentSetManager : MonoBehaviour 
{
	...
	public void RecalculateData()
	{
		...
		int startingEnvSet = Settings.GetInt("starting-envset", savedEnvSet);	//WhimsyWoodsId);
		
		// the following code doesn't actually do anything, but it did cause issue DMTRO-1979 when startingEnvSet was being set to WhimseyWoodsID here
		if (!ResourceManager.SharedInstance.IsAssetBundleAvailable(GetAssetBundleName(savedEnvSet))
			|| !ResourceManager.SharedInstance.IsAssetBundleDownloadedLastestVersion(GetAssetBundleName(savedEnvSet)))
			savedEnvSet = startingEnvSet;
		...
		{
			CurrentEnvironmentSet = AllDict[startingEnvSet];		
		}
	}
	...
	void EnvironmentStateChanged(EnvironmentSetSwitcher.SwitchState newState, int newEnvSetId)
	{
		if (newState == EnvironmentSetSwitcher.SwitchState.finished)
		{
			...
			if(ObjectivesDataUpdater.GetStatForObjectiveType(ObjectiveType.ReachLocation,1)<=0)
			...
			CurrentEnvironmentSet = LocalDict[newEnvSetId];
	...
	private void calcWhichAreLocal()
	{
		...
		for ( int i=0; i< EnvironmentSetBootstrap.BootstrapList.Count; i++)
		{
			...
			if ( addToLocal && AllCode2Dict.ContainsKey(boot.SetCode))
			{
				int setId = AllCode2Dict[boot.SetCode].SetId;
				LocalDict[setId] = AllCode2Dict[boot.SetCode];	
				LocalCode2Dict[boot.SetCode] = AllCode2Dict[boot.SetCode];	
...
//		Debug.LogError("---------Checking bundle " + bname + " result = " + result + " id = " + setId);
	...
	{
		// if everything is embedded it is already in Resources
		if (! Settings.GetBool("embed-all-envsets", false) )
		{
			if ( ! ResourceManager.SharedInstance.IsAssetBundleAvailable( GetAssetBundleName(CurrentEnvironmentSet.SetId))  &&
				  ! CurrentEnvironmentSet.IsEmbeddedAsResource() )
			{
				StartCoroutine( loadCurrentBundle(delg));
				return;			
			}
		}
		// if we get here it's probably embedded
/*		ResourceManager.AllowLoad = true;
		TrackPiece.WarmPrefabs();
		ResourceManager.AllowLoad = false;
		TrackPiece.WarmPools();
		EnvironmentSetSwitcher.SharedInstance.loadNewTrackMaterial(CurrentEnvironmentSet.SetId);
		EnvironmentSetSwitcher.SharedInstance.ReloadEnvAudio(CurrentEnvironmentSet.SetId);
		GameController.SharedInstance.NeededSkyBox = CurrentEnvironmentSet.SetId;
	...
	{
		yield return StartCoroutine( ResourceManager.SharedInstance.LoadAssetBundleCoroutine( GetAssetBundleName(CurrentEnvironmentSet.SetId), false, -1, IsEmbedded(CurrentEnvironmentSet.SetId)));

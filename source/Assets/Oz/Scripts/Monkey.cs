...
public class Monkey : MonoBehaviour {
	...
	public IEnumerator Disable()
	{
		...
		if(wandSmokeEffect != null) { wandSmokeEffect.gameObject.active=true;	wandSmokeEffect.Play(); }
		if(wandStarEffect != null)	{ wandStarEffect.gameObject.active=true;	wandStarEffect.Play(); }
	}
	...
	IEnumerator SpawnMegaCoin()
	{
		...
		while(megaCoin.transform.position!=GamePlayer.SharedInstance.transform.position/*targetpos*/ && megaCoin!=null && (megaCoin.gameObject.active||!megaCoinActivated))
		{
			targetpos = GamePlayer.SharedInstance.transform.position;
			
			if(!megaCoinActivated && megaCoin.gameObject.active)	megaCoinActivated = true;
	...
	IEnumerator SpawnGem()
	{
		...
		while(gem.transform.position!=GamePlayer.SharedInstance.transform.position/*targetpos*/ && gem!=null && (gem.gameObject.active||!gemActivated))
		{
			targetpos = GamePlayer.SharedInstance.transform.position;
	...
	IEnumerator OnTriggerEnter(Collider other)
	{
		...
		{
			if(GamePlayer.SharedInstance.HasBoost == false && GamePlayer.SharedInstance.HasGlindasBubble==false && GamePlayer.SharedInstance.HasPoof==false)
			{
				AttatchPlayer();

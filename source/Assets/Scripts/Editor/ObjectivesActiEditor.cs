...
// wxj
public class ObjectivesActiEditor : EditorWindow 
{
	...
	// wxj
	private List<ObjectiveProtoData> dataList;
	
	[MenuItem("TR2/Editors/Activity Objectives Editor")]
	static public void OpenEditor()
	{
		EditorWindow.GetWindow<ObjectivesActiEditor>(false, "Activity Objectives Editor", true);
	}
	...
	void OnGUI()
	{
		...
		if (dataList == null) 
		{
			dataList = new List<ObjectiveProtoData>();
			LoadFile();
		}
		...
		if (GUILayout.Button("Add") == true)
		{
			...
			ob._id = ObjectivesManager.GetNextID(dataList);
	...
	{
		bool success = ObjectivesManager.LoadFile(dataList, "OZGameData/ObjectivesActivity");
		if(success == false)
		{
			GUILayout.Label("Failed to load ObjectivesActivity.txt");
			notify.Debug("Failed to load ObjectivesActivity.txt");
			return;
		}
		else
		{
			dataList.Sort((a1, a2) => a1._id.CompareTo(a2._id));
		}		
	}
}

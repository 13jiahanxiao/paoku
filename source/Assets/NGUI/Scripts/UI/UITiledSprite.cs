...
public class UITiledSprite : UISlicedSprite
{
	...
	[HideInInspector][SerializeField] private Vector2 _tiling = new Vector2(1,1);
	public Vector2 tiling 
	{
		get { return _tiling; }
		set { _tiling = value; mChanged = true; }
	}
	/// <summary>
...
#endif
	{
		...
		Vector2 curTiling = _tiling;
		if(curTiling.x < 0.01f)
		{
			curTiling.x = 0.01f;
		}
		if(curTiling.y < 0.01f)
		{
			curTiling.y = 0.01f;
		}
		...
		float width  = Mathf.Abs(1f/curTiling.x);//Mathf.Abs(rect.width / scale.x) * pixelSize;
		float height = Mathf.Abs(1f/curTiling.y);//Mathf.Abs(rect.height / scale.y) * pixelSize;
		...
		if (width < 0.05f || height < 0.05f)
		{
			Debug.LogWarning("The tiled sprite (" + NGUITools.GetHierarchy(gameObject) + ") is too small.\nConsider using a bigger one.\n"
				+ "w: " + rect.width + " h: " + rect.height + " sx: " + scale.x + " sy: " + scale.y + " ps: " + pixelSize );

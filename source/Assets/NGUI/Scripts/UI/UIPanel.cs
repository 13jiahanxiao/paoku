...
public class UIPanel : MonoBehaviour
{
	...
	public int LastFills = 0;
	public int LastFillsFrame = 0;
	public int MaxFills = 0;
	public int LastGeomFills = 0;
	//public int LastFillsFrame = 0;
	public int MaxGeomFills = 0;
	public bool Reset = false;

	public static int _CurUpdates = 0;
	public static int _CurFills = 0;
	public static int _GeomFills = 0;
	public static int _LastUpdates = 0;
	public static int _LastFills = 0;
	public static int _LastGeomFills = 0;
	public static int _LastFillsFrame = 0;
	public static int _MaxFills = 0;
	public static int _MaxGeomFills = 0;
	public static int _CurFrame = 0;	
	
	public static int _ShowFrame = 0;
	...
	public void UpdateDrawcalls ()
	{
		...
		//with a flag for 'moved' when the panel transform changes we can bail early on platforms that do not change screen size
/*		else
		if((!mChangedLastFrame)&&(mChanged.size == 0))
		{
			return;
		}*/
	...
	void LateUpdate ()
	{
		if((mWidgets.size == 0)&&(!mWidgetsRemoved))
		{
			return;
		}
		mWidgetsRemoved = false;
		...
		bool show = (_ShowFrame == Time.frameCount);
		if(_CurFrame != Time.frameCount)
		{
			_LastUpdates = _CurUpdates;
			_CurUpdates = 0;
			if(_CurFills != 0)
			{
				_LastGeomFills = _GeomFills;
				_LastFills = _CurFills;
				_LastFillsFrame = _CurFrame;
			}
			if(_CurFills> _MaxFills)
			{
				_MaxGeomFills = _GeomFills;
				_MaxFills = _CurFills;
				Debug.Log("Fills " + _MaxFills + " " + _MaxGeomFills + " @ " + _CurFrame);
			}
			_CurFrame = Time.frameCount;
			_GeomFills = 0;
			_CurFills = 0;
		}
		if(Reset || ((Time.frameCount % 600) == 0))
		{
			_MaxFills = 0;
			_MaxGeomFills = 0;
			Reset = false;
		...
		LastFills = _LastFills;
		LastGeomFills = _LastGeomFills;
		LastFillsFrame = _LastFillsFrame;
		MaxFills = _MaxFills;
		MaxGeomFills = _MaxGeomFills;
							
		++_CurUpdates;
		if(show)
		{
			Debug.Log("LateUpdate " + _CurUpdates + " : " + gameObject.name + " : " + mWidgets.size + " @ " + Time.frameCount);
		}
#endif
		UpdateTransformMatrix();
		UpdateTransforms();
		...
		for (int i = 0, imax = mChanged.size; i < imax; ++i) 
		{
			...
			if(show)
			{
				Debug.Log("Fill " + gameObject.name + " " + mChanged.buffer[i].name + " @ " + Time.frameCount);
			}
			++_CurFills;
			_GeomFills += 
#endif

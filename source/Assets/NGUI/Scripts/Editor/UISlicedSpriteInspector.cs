...
public class UISlicedSpriteInspector : UISpriteInspector
{
	...
	override protected bool OnDrawProperties ()
	{
		if (base.OnDrawProperties())
		{
			...
			float cornerMult = EditorGUILayout.FloatField("CornerMult", sp.cornerMult);
			if (sp.cornerMult != cornerMult)
			{
				NGUIEditorTools.RegisterUndo("Sprite Change", sp);
				sp.cornerMult = cornerMult;
				EditorUtility.SetDirty(sp.gameObject);
			}
			
			/*
			int atlasSize = EditorGUILayout.IntField("AtlasSize", sp.atlasSize);
			if (sp.atlasSize != atlasSize)
			{
				NGUIEditorTools.RegisterUndo("Sprite Change", sp);
				sp.atlasSize = atlasSize;
				EditorUtility.SetDirty(sp.gameObject);
			}
			*/
			return true;

...
public abstract class UIWidget : MonoBehaviour
{
...
#if DEBUG_GEOMUPDATE
	public int LastGeomUpdates = 0;
	public int LastGeomUpdatesFrame = 0;
	public int MaxGeomUpdates = 0;
	public bool Reset = false;

	public static int _CurFrame = 0;
	public static int _CurGeomUpdates = 0;
	public static int _LastGeomUpdates = 0;
	public static int _LastGeomUpdatesFrame = 0;
	public static int _MaxGeomUpdates = 0;
	...
	public bool UpdateGeometry (ref Matrix4x4 worldToPanel, bool parentMoved, bool generateNormals)
	{
		...
		{
			if(_CurGeomUpdates != 0)
			{
				_LastGeomUpdates = _CurGeomUpdates;
				_LastGeomUpdatesFrame = _CurFrame;
			}
			if(_CurGeomUpdates> _MaxGeomUpdates)
			{
				_MaxGeomUpdates = _CurGeomUpdates;
				Debug.Log("Geom updates " + _MaxGeomUpdates + " @ " + _CurFrame);
			}
		...
		if(Reset || ((Time.frameCount % 600) == 0))
		{
			...
			++_CurGeomUpdates;
		LastGeomUpdates = _LastGeomUpdates;
		LastGeomUpdatesFrame = _LastGeomUpdatesFrame;
		MaxGeomUpdates = _MaxGeomUpdates;
			
		Debug.Log ("Geom updated for " + gameObject.name + " " + mChanged + " @ " + Time.frameCount);
#endif							

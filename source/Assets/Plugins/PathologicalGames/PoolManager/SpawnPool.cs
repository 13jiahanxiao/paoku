...
public sealed class SpawnPool : MonoBehaviour, IList<Transform>
{
	...
	{
		ActivateNextDeferredActivate();
	}

	public void ActivateDeferred(Vector3 position, float radiusSquared)
	{
		ActivateNextDeferredActivate(position, radiusSquared);
	}
    ...
    public List<Transform> CreatePrefabPool(PrefabPool prefabPool)
    {
        ...
        if (!isAlreadyPool)
        {
			...
			{
				Debug.LogWarning("Adding prefab pool with same name as existing prefab pool " + prefabPool.prefab.name);
			}
			else
			{
    ...
    public IEnumerator CreatePrefabPoolCoroutine(PrefabPool prefabPool, List<Transform> instances)
    {
        // Only add a PrefabPool once. Uses a GameObject comparison on the prefabs
        //   This will rarely be needed and will almost Always run at game start, 
        //   even if user-executed. This really only fails If a user tries to create 
...
//        List<Transform> instances = new List<Transform>();
        if (prefabPool.preloaded != true)
        {
			...
			yield return StartCoroutine(prefabPool.PreloadInstancesCoroutine(prefabs));
            instances.AddRange(prefabs);
...
//		return instances;
    ...
    #region Deferred Activate
/*	private class DeferredActivate
	{
		public delegate void OnDeferredActivate(GameObject obj, Object data);
		OnDeferredActivate onDeferredActivate;
		GameObject obj;
		Object data;
	}*/
	
	private List<GameObject> DeferredActivateList = null;
	
	public void AddDeferredActivate(GameObject obj)
	{
		if(DeferredActivateList == null)
		{
			DeferredActivateList = new List<GameObject>();
		}
		DeferredActivateList.Add(obj);
	}

	public bool ActivateNextDeferredActivate()
	{
		if((DeferredActivateList == null)||(DeferredActivateList.Count == 0))
		{
			return false;
		}
		GameObject obj = DeferredActivateList[0];
		DeferredActivateList.RemoveAt(0);
		obj.SetActiveRecursively(true);
		return true;
	}

	public bool ActivateNextDeferredActivate(Vector3 center, float radiusSquared)
	{
		if((DeferredActivateList == null)||(DeferredActivateList.Count == 0))
		{
			return false;
		}
		foreach(GameObject obj in DeferredActivateList)
		{
			Vector3 offset = obj.transform.position - center;
			if(offset.sqrMagnitude < radiusSquared)
			{
				DeferredActivateList.Remove(obj);
				obj.SetActiveRecursively(true);
				return true;
			}
		}
	...
	public bool RemoveDeferredActivate(GameObject obj)
	{
		if((DeferredActivateList == null)||(DeferredActivateList.Count == 0))
		{
			return false;
		}
		return DeferredActivateList.Remove(obj);
	}
	#endregion Deferred Activate
    ...
    /// </returns>
    //
    public Transform Spawn(Transform prefab, Vector3 pos, Quaternion rot, bool deferActivate = false)
    {
        PrefabPool pool = GetPoolForPrefab(prefab);
		if(pool!=null)
			return SpawnFromPool(pool,pos,rot,deferActivate);
    ...
    /// used the limit option.
    /// </returns>
    public Transform Spawn(Transform prefab, bool deferActivate = false)
    {
        return this.Spawn(prefab, Vector3.zero, Quaternion.identity, deferActivate);
    }
	
	
	public PrefabPool GetPoolForPrefab(Transform prefab)
	{
	//	Transform inst;
	...
	public Transform SpawnFromPool(PrefabPool pool, Vector3 pos, Quaternion rot, bool deferActivate = false)
	{
        ...
        Transform inst = pool.SpawnInstance(pos, rot, deferActivate);
		if(deferActivate)
		{
			AddDeferredActivate(inst.gameObject);
		}
    ...
    public void Despawn(Transform xform, PrefabPool prefabPool)
    {
		...
		if(prefabPool == null) {
			for (int i = 0; i < max; i++) {
	            ...
	            {
	                despawned = prefabPool.DespawnInstance(xform,!RemoveDeferredActivate(xform.gameObject));
	                break;
    ...
    /// <summary>
    /// Get the prefab pool that manages the passed in prefab
    /// </summary>
    /// <param name="prefab">The GameObject of an prefab (not an instance)</param>
    ...
    /// <summary>
    /// Delete a prefab pool. WARNING you better know what you're doing, initially used in EnvironmentSetSwitcher
    /// </summary>
    /// <returns>true if deletion successful</returns>
	public bool DeletePrefabPool( PrefabPool prefabPoolToDelete, string prefabName)
	{
		bool result = false;
		foreach (PrefabPool prefabPool in this._prefabPools)
		{
			if (prefabPool == prefabPoolToDelete)
			{
				result = true;
				this._prefabPools.Remove(prefabPoolToDelete);
				
				// we also need to delete the corresponding entry in .prefabs
				this.prefabs.Remove(prefabName);
				break;
			}
		}
		return result;
...
public class PrefabPool
{
    ...
    /// </returns>
    internal bool DespawnInstance(Transform xform, bool deactivate = true)
    {
        if (this.logMessages)
    ...
    /// reached.
    /// </returns>    
    internal Transform SpawnInstance(Vector3 pos, Quaternion rot, bool deferActivate)
    {
        Transform inst;
    ...
    internal Transform SpawnNew(Vector3 pos, Quaternion rot)
    {
		...
		{
			Debug.Log ("null spawn pool " + this.prefab.name);	
		}
		
		if (this.spawnPool.group == null)
		{
			Debug.Log ("group is null" + this.prefab.name);
		}
    ...
    internal IEnumerator PreloadInstancesCoroutine(List<Transform> instances)
    {
        // Define here to return empty if needed by the folowing errors
        //List<Transform> instances = new List<Transform>();
        ...
        while (this.totalCount < this.preloadAmount) // Total count will update
        {
            ...
            if (inst == null)
            {
				Debug.Log("Failed to create instance @ " + Time.frameCount);
                Debug.LogError(string.Format("SpawnPool {0} ({1}): " +
                                    "You turned ON 'Limit Instances' and entered a " +
                                    "'Limit Amount' greater than the 'Preload Amount'!",
                                 this.spawnPool.poolName,
...
public class PrefabsDict : IDictionary<string, Transform>
{
    ...
    {
        bool result = this._prefabs.Remove(prefabName);
		return result;
    }
    ...
    { 
		//Dangerous territory but we need this for memory reasons
        //throw new System.NotImplementedException("Read-Only"); 
